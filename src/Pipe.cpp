...
Pipe::Pipe(Type type, const TextureHolder &textures, sf::Vector2f position,
           sf::Vector2f velocity, bool dir)
    : Entity(true), mSprite(textures.get(toTextureID(type))), mType(type),
      isOut(false) {
  move(position);
  if (dir)
...
unsigned int Pipe::getCategory() const { return Category::Pipe; }

sf::FloatRect Pipe::getBoundingRect() const {
  return getWorldTransform().transformRect(mSprite.getGlobalBounds());
}

bool Pipe::getStatus() { return isOut; }
...
void Pipe::drawCurrent(sf::RenderTarget &target,
