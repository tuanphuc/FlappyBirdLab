...
Genome::Genome(std::vector<unsigned int> architecture)
    : mNeuron{0.0, {}}, mArch(architecture), mScore(0) {
...
void Genome::updateScore(unsigned int score) { mScore = score; }

unsigned int Genome::getScore() { return mScore; }
...
void Genome::populateLayer(Layer &layer, unsigned int nbNeuron,
...
void Genome::populateNetwork(Network &network,
                             const std::vector<unsigned int> &mArch) {
  // Add input layer
