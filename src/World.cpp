...
World::World(sf::RenderWindow &window, CommandQueue &commandQueue,
             unsigned int nBird)
    : mWindow(window), mWorldView(window.getDefaultView()), mTextures(),
      mSceneGraph(), mSceneLayers(), mCommandQueue(commandQueue),
      mBackgroundX(0), mBackground(nullptr), mPipe(nullptr), mDeltaBord(50.f),
      mPipeHole(120.f), mSpawnRate(0.f),
      mPipeVelocity(sf::Vector2f(-180.f, 0.f)), mGameSpeed(MIN_GAME_SPEED) {
  loadTextures();
  buildScene(nBird);
}

World::World(sf::RenderWindow &window, CommandQueue &commandQueue)
    : World(window, commandQueue, 0) {}
...
void World::reset(unsigned int nBird) {
  ...
  command.action = derivedAction<Entity>([this](Entity &e) { e.die(); });
...
void World::spawnHoles(sf::Time dt) {
  if (mSpawnRate > 0)
    ...
    int mHolePosition =
        (int)(r * (512 - mDeltaBord * 2 - mPipeHole) + mDeltaBord);

    double d = ((double)rand() / (RAND_MAX));
    bool direction = (d >= 0.5);
    std::unique_ptr<Pipe> pipeTop(new Pipe(
        Pipe::PipeTop, mTextures, sf::Vector2f(1000.f, mHolePosition - 512.f),
        mPipeVelocity, direction));
    mSceneLayers[PipeLayer]->attachChild(std::move(pipeTop));
    std::unique_ptr<Pipe> pipeBottom(
        new Pipe(Pipe::PipeBottom, mTextures,
                 sf::Vector2f(1000.f, mHolePosition + mPipeHole), mPipeVelocity,
                 direction));
...
void World::handleCollision() {
  ...
  mSceneLayers[BirdLayer]->checkCollisionWith(mSceneLayers[PipeLayer],
                                              collisionPairs);
