...
LearningState::LearningState(StateStack &stack, Context context)
    : State(stack, context), mPopulation(50), mAliveBirds(50),
      mGenerationCounter(0),
      mWorld(*context.window, mCommandQueue, mPopulation),
      mScenePointer(mWorld.getScene()), mGeneticLab(mPopulation),
      mPlayer(*context.player), mLivingBirds(mPopulation, true), mScore(0),
      mFonts(), mStatisticsText(), mStatisticsUpdateTime() {
...
bool LearningState::update(sf::Time dt) {
  ...
  if (!mWorld.hasAlivePlayer()) {
    ...
    auto positionBirds =
        mScenePointer[World::BirdLayer]->getWorldPositionChildren();
    auto positionPipes =
        mScenePointer[World::PipeLayer]->getWorldPositionChildren();
    ...
    for (unsigned int i = 0; i < mLivingBirds.size(); ++i) {
      if (mLivingBirds.at(i)) {
        ...
        index += 1;
        if (mGeneticLab.getGeneration().back().at(i).computeNetwork(inputs) >
            0.5) {
