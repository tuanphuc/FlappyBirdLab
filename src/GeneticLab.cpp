...
GeneticLab::GeneticLab(std::vector<unsigned int> architecture,
                       unsigned int population)
    : mElitism(0.2), mPopulation(population), mNbChildren(1),
      mRandomBehaviour(0.2), mCrossoverRate(0.5), mMutationRate(0.1),
      mMutationRange(0.5), mHistoric(0), mScores(), mArch(architecture) {
...
GeneticLab::GeneticLab(unsigned int population)
    : GeneticLab({2, 2, 1}, population) {}
...
void GeneticLab::update(unsigned int highestScore) {
  ...
  std::sort(mGenerations.back().begin(), mGenerations.back().end(),
            Genome::customGreater);
  generateNextGeneration();
}

GeneticLab::Generation &GeneticLab::getGeneration() { return mGenerations; }

void GeneticLab::setScore(unsigned int index, unsigned int score) {
  mGenerations.back().at(index).updateScore(score);
}

unsigned int GeneticLab::getMaxScore() {
  if (mScores.empty())
    return 0;
  else
...
std::vector<Genome> GeneticLab::breed(Genome g1, Genome g2,
  ...
  for (unsigned int i = 0; i < nbChilds; ++i) {
    ...
    for (auto &weight : flatten1)
      if ((double)rand() / RAND_MAX <= mMutationRate)
        weight +=
            (double)rand() / RAND_MAX * mMutationRange * 2 - mMutationRange;
...
void GeneticLab::generateNextGeneration() {
  ...
  for (unsigned int i = 0; i < (unsigned int)round(mElitism * mPopulation);
       ++i) {
  ...
  for (unsigned int i = 0;
       i < (unsigned int)round(mRandomBehaviour * mPopulation); ++i) {
    if (nextGeneration.size() < mPopulation) {
      Genome genome(mArch);
  ...
  while (true) {
    ...
    for (unsigned int i = 0; i < max; ++i) {
      auto childs = breed(mGenerations.back().at(i),
                          mGenerations.back().at(max), mNbChildren);
