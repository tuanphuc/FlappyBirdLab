...
MenuState::MenuState(StateStack &stack, Context context)
  ...
  auto learningButton =
      std::make_shared<GUI::Button>(*context.fonts, *context.textures);
  learningButton->setPosition(playButton->getPosition() +
                              sf::Vector2f(0.f, 60.f));
  learningButton->setText("Learning");
  learningButton->setCallback([this]() { requestStackPush(States::Learning); });

  auto exitButton =
      std::make_shared<GUI::Button>(*context.fonts, *context.textures);
  exitButton->setPosition(learningButton->getPosition() +
                          sf::Vector2f(0.f, 60.f));
